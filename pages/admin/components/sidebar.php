...
?>
    ...
    <aside id="sidebarMenu"
      ...
      <div class="flex flex-col px-4 py-6 gap-0 flex-1 overflow-y-auto sidebar-scroll relative z-10">
        ...
        <!-- ---- Laporan (Dropdown) ---- -->
        <?php $is_laporan_open = ($active_page == 'laporan' || $active_page == 'stock'); ?>
        <div class="mb-1">
          ...
          <div id="laporan-submenu" class="submenu <?php echo $is_laporan_open ? 'open' : ''; ?>">
            <div class="ml-[25px] pl-4 py-1 mt-1 mb-2 flex flex-col gap-1.5 border-l-[1.5px] border-white/20">
              ...
              <a href="index.php?page=stock"
                class="block px-3 py-2.5 rounded-xl font-plex text-[15px] leading-5 transition-all duration-200 <?php echo ($active_page == 'stock') ? 'bg-white/10 border border-white/20 shadow-sm text-white font-medium' : 'border border-transparent hover:bg-white/5 text-white/70 hover:text-white font-normal'; ?>">
                Laporan Stok Masuk
              </a>
            </div>
          </div>
